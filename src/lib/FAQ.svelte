<script>
  ...
  export let qas = [
    {
      q: 'Do we choose our outfits?',
      a: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque, delectus!'
    },
    {
      q: 'What locations can I pick?',
  ...
  function questionClicked({q, a}, stopAuto = false) {
    answer = a
    question = q
...
<section>
  <div>
    ...
    <div class="questions">
      {#each qas as qa}
        <button on:click={questionClicked(qa, true)} class:active={question === qa.q}>
          {qa.q}
        </button>
      {/each}
    </div>
